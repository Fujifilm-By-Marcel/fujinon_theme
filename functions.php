...
function add_item( $admin_bar ){
  global $pagenow;
  if ( is_admin() ) {
    $admin_bar->add_menu( array( 'id'=>'rebuild-product-data','title'=>'Rebuild Product Data','href'=>'#' ) );
  }
}

/* Here you trigger the ajax handler function using jQuery */
add_action( 'admin_footer', 'rebuild_product_data_action_js' );

function rebuild_product_data_action_js() { ?>
  <script type="text/javascript" >
     jQuery("li#wp-admin-bar-rebuild-product-data .ab-item").on( "click", function() {
     	console.log('test');
        var data = {
                      'action': 'rebuild_product_data',
                    };
...
add_action( 'wp_ajax_rebuild_product_data', 'rebuild_product_data_callback' );

function rebuild_product_data_callback() {
	do_action('buildProductData');
	$response = "Product rebuild started. Check cron for progress. Do not run this again until the cron has finished working.";
    echo $response;
