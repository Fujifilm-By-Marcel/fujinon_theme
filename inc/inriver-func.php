...
class Inriver {
	...
	private function curlInriver($isPost, $url, $postFields){
		...
		// Define the constant
        $inriver_api_key = defined('INRIVER_API_KEY') ? INRIVER_API_KEY : '';

		$header = array (	
		    //'X-inRiver-APIKey: ' . $_ENV["PIMAPIKEY"],
		    'X-inRiver-APIKey: ' . $inriver_api_key,
	...
	public function add_product($var){	
		...
		$var->shortDescription = $this->find_object_by_id($linksandfields->fields, "ProductShortDescription")[0]->en;
		...
		$my_post = array(
		  ...
		  'post_excerpt' => $var->shortDescription,
